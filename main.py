...
class AdaptiveGammaSqueezeSystem:
    ...
    async def _apply_parameter_adjustments(self, learning_result: Dict):
        ...
        for param_path, adjustment in adjustments.items():
            try:
                ...
                # Get current value
                current_value = self._get_current_value(param_path)
                if current_value is None:
                    logger.warning(f"Could not get current value for {param_path}")
                    continue
                
                # Calculate new value (adjustment is a delta)
                new_value = current_value + adjustment
                
                # Ensure data type consistency
                if param_path in self.integer_params:
                    new_value = int(round(new_value))
                
                # Check bounds
                bounds = self.config['adaptive_learning']['parameter_bounds'].get(param_path)
                if bounds:
                ...
                if parts[0] == 'market_behavior' and hasattr(self.behavior_detector, 'update_parameters'):
                    update_dict = self._build_nested_dict('.'.join(parts[1:]), new_value)
                    if self.behavior_detector.update_parameters(update_dict):
                        success_count += 1
                        logger.info(f"✓ Updated {param_path}: {current_value:.3f} → {new_value:.3f} (Δ{adjustment:+.3f})")
    ...
    def _get_current_value(self, param_path: str) -> Optional[float]:
        """Get current parameter value"""
        parts = param_path.split('.')
        
        # Get the config from the appropriate module
        if parts[0] == 'market_behavior':
            config = self.behavior_detector.config
            parts = parts[1:]
        elif parts[0] == 'gamma_analysis':
            config = self.gamma_analyzer.config
            parts = parts[1:]
        elif parts[0] == 'signal_generation':
            config = self.signal_evaluator.config
            parts = parts[1:]
        else:
            return None
    ...
    def _print_learning_report(self, report: Dict):
        ...
        # Bootstrap status
        bootstrap = report.get('bootstrap_status', {})
        if bootstrap.get('active'):
            logger.info(f"\nBootstrap Status:")
            logger.info(f"  Active: Yes")
            logger.info(f"  Cycles Completed: {bootstrap.get('cycles_completed')}/{bootstrap.get('min_required')}")
        ...
        while self.running:
            try:
                ...
                if learner_decisions:
                    avg_quality = np.mean([d.get('decision_quality', 0.5) for d in list(learner_decisions)[-5:]])
